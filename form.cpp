    ...
    ui(new Ui::Form)
{
    m_combo_count = 10;
    ui->setupUi(this);
//    ui->comboBoxSearch->setMaxCount(m_combo_count);
...
//    connect(ui->productsListWidget, SIGNAL(currentRowChanged(int)), this, SLOT(showProductImage(int)));
...
void Form::keyPressEvent(QKeyEvent* ev)
{
    if (ev->key() == Qt::Key_Return)
    {
        ...
        {
            qDebug() << __FUNCTION__ << "Start search";
            startSearch();
        }
        else if (active == (QWidget*)ui->twProducts)
        {
            qDebug() << __FUNCTION__ << "Show details";
            showDetails();
        }
    }
...
void Form::showDetails()
{
    ...
    ProductDetailDlg* uiDialog = new ProductDetailDlg(this);

    uiDialog->data()->labelTitle->setText(products[pos].toMap()["product"].toMap()["title"].toString());
    uiDialog->data()->teDescription->setText(products[pos].toMap()["product"].toMap()["description"].toString());
    ...
    uiDialog->data()->twDetails->setColumnCount(1);
    uiDialog->data()->twDetails->setColumnWidth(0,150);

    QString str = products[pos].toMap()["product"].toMap()["author"].toMap()["name"].toString();
    itemAuthor = new QTableWidgetItem(str);
    uiDialog->data()->twDetails->setItem(0,0,itemAuthor);
    ...
    uiDialog->data()->twDetails->setItem(1,0,itemTimeCreate);
    ...
    uiDialog->data()->twDetails->setItem(2,0,itemTimeModify);
...
{
    qDebug() << __FUNCTION__ << "row=" << row << ", curtable row=" << column;
//    ui->twProducts->setCurrentIndex(index);
//    ui->twProducts->setCurrentCell(index.row(),0);
    showDetails();
}
